...
The repository contains the full sweep, experiment, and build scripts. The literature matrix is in \texttt{docs/related\_work\_matrix.csv}. Experiment outputs are in \texttt{results/} and figures in \texttt{figures/}. The v2 stress table is generated from \texttt{results/signature\_budget\_stress.csv}. The ICLR template fetch status was \texttt{official\_template\_ready}; source details are written to \texttt{paper/template\_source.md}.
